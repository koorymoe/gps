...
import { searchCustomerSubscription, renewSubscription } from '@/lib/firebase/firestore'
...
export default function RenewalsPage() {
  ...
  async function handleRenew() {
    ...
    try {
      await renewSubscription(found.requestId as string, newSub, found.subscription_end as string)
      setSuccess(true)
    } catch { setError('حدث خطأ أثناء التجديد') }
    setLoading(false)
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="card mb-6">
        ...
        <div className="flex gap-3">
          <input className="input-field flex-1" value={search} onChange={e => setSearch(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSearch()} placeholder="اسم الزبون أو رقم الهاتف..." />
          <button onClick={handleSearch} className="btn-secondary px-6 py-2 w-auto" disabled={loading}>{loading ? '...' : 'بحث'}</button>
        </div>
      ...
      {found && (
        <div className="card space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm p-4 rounded-xl bg-gray-50">
            ...
            <div>
              ...
              <span className="font-semibold" style={{ color: getRemainingDays(found.subscription_end as string) < 0 ? '#dc2626' : '#16a34a' }}>
                {getStatusLabel(getSubscriptionStatus(found.subscription_end as string))}
              </span>
            </div>
          </div>
          <div>
          ...
          <button onClick={handleRenew} className="btn-primary" disabled={loading}>{loading ? 'جارٍ التجديد...' : '✅ تجديد الاشتراك'}</button>
        </div>
      )}
    </div>
  )
